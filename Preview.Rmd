...
```{r}
Glasgowlm <- lm(MEDPRICE~MEDROOM+CRIME+SALES+TYPE+INCOMEDEP, data=Glasgow.df)
summary(Glasgowlm)
```
...
```{r}
resGlas <- residuals(lm(MEDPRICE~MEDROOM+CRIME+SALES+TYPE+INCOMEDEP, data=Glasgow.df))
...
```{r}
#iiplots <- influenceIndexPlot(Glasgowlm, vars=c("Cook", "Studentized", "Bonf", "hat"),    main="Diagnostic Plots", labels=NULL, id.n=4, id.method = "y")
