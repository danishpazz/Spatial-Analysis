...
The Shapiro Wilk test is used to test if the residuals are normally distributed. The null hypothesis states that the residuals are normally distributed, hence a small p-value indicates that we should reject the null and conclude the residuals are not normally distributed.  In addition, the p value needs to be above 0.05.  As a result, the residuals are not normally distributed.

```{r}
resGlas <- residuals(lm(MEDPRICE~MEDROOM+CRIME+SALES+TYPE+INCOMEDEP, data=Glas.frame))
...
I would argue that CRIME and SALES might need to be transformed.  I would also like to look more into MEDROOM.
...
```{r}
iplots <- influenceIndexPlot(Glasgowlm, vars=c("Cook", "Studentized", "Bonf", "hat"),    main="Diagnostic Plots", labels=NULL, id.n=4, id.method = "y")

```

In hat-values and Cook’s distance, we can identifity 119 as an outlier. The influential cases include variable 10, 34, 41, and 150.119 appears to be extreme because of CRIME being at over 8000.  The other outliers did not seem too extreme when looking at them.Lets try dropping 119 as its illustrates extreme value. 


```{r}
Glasgowlm_log <- lm(log(MEDPRICE) ~ MEDROOM+CRIME+SALES+TYPE+INCOMEDEP, data=Glas.frame)
summary(Glasgowlm_log)
vif(Glasgowlm_log)
resGlas_log <- residuals(lm(log(MEDPRICE) ~ MEDROOM+CRIME+SALES+TYPE+INCOMEDEP, data=Glas.frame))
...
```{r}
Glasgowlm_log2 <- lm(log(MEDPRICE) ~ log(MEDROOM)+CRIME+SALES+TYPE+log(INCOMEDEP), data=Glas.frame)
...
```{r}
Glasgowlm_log3 <- (lm(log(MEDPRICE) ~ MEDROOM + CRIME + SALES + I(SALES^0.5) +TYPE+ INCOMEDEP  + I(INCOMEDEP^0.5), data=Glas.frame))
summary(Glasgowlm_log3)
resGlas_log3 <- residuals(lm(log(MEDPRICE) ~ MEDROOM + CRIME + SALES + I(SALES^0.5) +TYPE+ INCOMEDEP  + I(INCOMEDEP^0.5), data=Glas.frame))
shapiro.test(resGlas_log3)
residualPlots(Glasgowlm_log3)
```

```{r}
summary(powerTransform(cbind(MEDPRICE, MEDROOM, CRIME, SALES, INCOMEDEP) ~ 1, data = Glas.frame))
```

```{r}

Glas_subset <- as.data.frame(subset(Glasgow.df, select = - c(ID, INTGEOCODE, INTGEONAME)))
Glas_Numsub <- lapply(Glas_subset, as.numeric)
lapply(Glas_Numsub, class)
lmmodel <- lm(MEDROOM ~ . , data = Glas_subset)
summary(lmmodel)
```

 
Glas_subset[, c("FEM2001", "MALE2001", "POP2001", "NOFDWELL", "NOFSALES", "MEDROOM", "ASSAULT", "OBSRES2009", "OBSRES2009")] <-
  lapply(Glas_subset[, c("FEM2001", "MALE2001", "POP2001", "NOFDWELL", "NOFSALES", "MEDROOM", "ASSAULT", "OBSRES2009", "OBSRES2009")],
  function(x) as.numeric(levels(x))[x])
