...
```{r}
Glasgowlm_log3 <- (lm(MEDPRICE ~ MEDROOM + I(MEDROOM ^2) + CRIME + SALES + TYPE+ INCOMEDEP + (INCOMEDEP^2), data=Glas.frame))
...
lamda.a <- -0.3; lamda.MEDR <- -1; lamda.SALE <- 0.5 ; lamda.INC <- 0.33 

Glasgowlm_log4 <- (lm(MEDPRICE ~ bcPower(MEDROOM, lamda.MEDR)+ MEDROOM +  log(CRIME) + bcPower(SALES, lamda.SALE) + SALES + TYPE +bcPower(INCOMEDEP, lamda.INC)+ INCOMEDEP, data=Glas.frame))
summary(Glasgowlm_log4)

```

```{r}
Glasgowlm_OType <- (lm(MEDPRICE ~ bcPower(MEDROOM, lamda.MEDR)+ MEDROOM +  log(CRIME) + bcPower(SALES, lamda.SALE) + SALES + bcPower(INCOMEDEP, lamda.INC)+ INCOMEDEP, data=Glas.frame))
summary(Glasgowlm_OType)
```

```{r}
anova(Glasgowlm_OType, Glasgowlm_log4)
```

As pvalue is greater than 0.05, we 

```{r}
Glasgowlm_OCrime <- (lm(MEDPRICE ~ bcPower(MEDROOM, lamda.MEDR)+ MEDROOM +  TYPE + bcPower(SALES, lamda.SALE) + SALES + bcPower(INCOMEDEP, lamda.INC)+ INCOMEDEP, data=Glas.frame))
summary(Glasgowlm_OCrime)
```

```{r}
anova(Glasgowlm_OCrime, Glasgowlm_log4)
```

so we fail reject both null hypothesis. So that CRIME is significant in the model.

Now, will look again reagrding teh residuals. 
```{r}
#residualPlots(Glasgowlm_log5)
```
